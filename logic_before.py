...
class Device:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'user_info': {
                'id': self._get_device_user_id(),
                'name': self._get_device_user_name(),
                'email': self._get_device_user_email()
            }
        }
    
    def _get_device_user_id(self) -> UUID:
        return self.user_info.user_id
    
    def _get_device_user_name(self) -> str:
        return self.user_info.name
    
    def _get_device_user_email(self) -> str:
        return self.user_info.email
    
    @staticmethod
    def _is_device_status_valid(status: str) -> bool:
        return status in ['PENDING ACTIVATION', 'ACTIVE', 'INACTIVE']
